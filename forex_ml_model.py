"""
forex_ml_model.py — Forex ML Model Training & Prediction
=========================================================
Adapted from ml_model.py for forex pair signal prediction.
Supports single models and ensemble methods with walk-forward
purged cross-validation, feature selection, and adaptive confidence.

Identical ML architecture to the stock model — the ensemble stacking
of XGBoost/LightGBM/CatBoost is market-agnostic. Only the label
generation and prediction interpretation differ for forex.
"""
...
class ForexMLModel:
    """
    ML model for forex signal prediction.
    ...
    def __init__(self, config_path: str = "forex_config.yaml"):
        ...
        self.model_dir = Path("models/forex")
        ...
        self.max_features = model_cfg.get("max_features", 35)
        self.feature_selection_mode = model_cfg.get("feature_selection_mode", "global")
        self.purge_gap = model_cfg.get("purge_gap_bars", 48)
        self.embargo_bars = model_cfg.get("embargo_bars", 12)
        self.use_sample_weights = model_cfg.get("use_sample_weights", True)
        self.use_smote = model_cfg.get("use_smote", False)
        self.min_probability_gap = model_cfg.get("min_probability_gap", 0.10)
    ...
    def _create_single_model(self, model_type: str, params: dict | None = None):
        ...
        if model_type == "xgboost":
            ...
            default_params = {
                ...
                "reg_alpha": 0.1, "reg_lambda": 1.0, "objective": "multiclass",
    ...
    def _create_model(self, params: dict | None = None):
        """Create model based on ensemble configuration."""
    ...
    def _select_features(self, X: pd.DataFrame, y: np.ndarray, force_refit: bool = False) -> pd.DataFrame:
        ...
        if not force_refit and self.feature_selection_mode == "global" and self.selected_features:
        ...
        try:
            ...
            importances = pd.Series(
                selector_model.feature_importances_, index=X.columns,
    ...
    def train(self, X: pd.DataFrame, y: pd.Series, params: dict | None = None) -> dict:
        ...
        if getattr(self, "use_smote", False):
            try:
                ...
                if counts.min() > 5:
                    ...
                    logger.info(f"SMOTE: {len(X_selected)} → {len(X_fit)} samples")
            except Exception as e:
    ...
    def walk_forward_validate(
        ...
        min_train_size: int = 500, params: dict | None = None,
        purge_gap: int | None = None, embargo: int | None = None,
    ) -> dict:
        """Walk-forward cross-validation with purge gap (prevents lookahead bias)."""
        effective_purge = purge_gap if purge_gap is not None else max(self.purge_gap, 48)
        effective_embargo = embargo if embargo is not None else max(self.embargo_bars, 12)
        ...
        for fold in range(n_splits):
            ...
            if buy_label is not None:
                buy_precision = precision_score(
                    y_test_enc, y_pred_enc, labels=[buy_label],
                    average="micro", zero_division=0
                )

            fold_info = {
                "fold": fold + 1, "train_size": len(X_train),
                "test_size": len(X_test), "accuracy": round(acc, 4),
                "f1_weighted": round(f1, 4), "buy_precision": round(buy_precision, 4),
            }
        ...
        if buy_label is not None:
            ...
            win_rate = (y_true_arr[buy_mask] == buy_label).mean() if buy_mask.sum() > 0 else 0.0
        else:
            win_rate = 0.0

        results = {
            "n_splits": n_splits, "purge_gap_bars": effective_purge,
        ...
        logger.info(f"Walk-forward: acc={overall_acc:.4f}, f1={overall_f1:.4f}, "
    ...
    def tune_hyperparameters(
        ...
        n_trials: int = 30, n_splits: int = 3,
        study_name: str = "forex_study",
    ) -> dict:
        """Tune hyperparameters with Optuna (persisted in SQLite)."""
        import optuna
        ...
        logger.info(f"📊 Optuna study '{study_name}' ({existing_trials} prior trials)")
        logger.info(f"🚀 Running {n_trials} tuning trials...")
    ...
    def predict(self, X: pd.DataFrame, confidence_threshold: float = 0.42) -> pd.DataFrame:
        ...
        min_gap = getattr(self, "min_probability_gap", 0.10)
        if min_gap is None:
            min_gap = 0.10
    ...
    def log_prediction_outcome(self, predicted: int, actual: int, confidence: float):
        ...
        window = self.config.get("monitoring", {}).get("rolling_accuracy_window", 100)
    ...
    def get_adaptive_threshold(self) -> float | None:
        """Compute adaptive threshold based on recent accuracy."""
        monitoring_cfg = self.config.get("monitoring", {})
        if not monitoring_cfg.get("enabled", False):
            return None

        window = monitoring_cfg.get("rolling_accuracy_window", 100)
        ...
        base_threshold = self.config["strategy"].get("confidence_threshold", 0.42)
        training_acc = self.training_metrics.get("train_accuracy", 0.5)

        if training_acc > 0 and accuracy < training_acc * 0.8:
            decay = (training_acc - accuracy) / training_acc
            adjusted = base_threshold + decay * 0.15
            self._adaptive_threshold = min(adjusted, 0.75)
        elif accuracy > training_acc * 1.1:
            self._adaptive_threshold = max(base_threshold - 0.05, 0.30)
        else:
            self._adaptive_threshold = base_threshold
    ...
    def get_model_health(self) -> dict:
        ...
        if len(actionable) < 5:
            return {"status": "insufficient_actionable", "total": len(recent)}

        accuracy = sum(1 for p in actionable if p["predicted"] == p["actual"]) / len(actionable)
        training_acc = self.training_metrics.get("train_accuracy", 0.5)
        decay_pct = ((training_acc - accuracy) / training_acc * 100) if training_acc > 0 else 0

        threshold = monitoring_cfg.get("decay_threshold_pct", 10)
        status = "degraded" if decay_pct > threshold else "healthy"
    ...
    def save(self, name: str = "forex_trading_model"):
    ...
    def load(self, name: str = "forex_trading_model"):
